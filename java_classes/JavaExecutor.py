...
class JavaExecutor(object):
    ...
    def startExecuting(self, classname, javafile, params):
        ...
        if compres.find("error") == -1 or compres.find("Exception") == -1:  # if building was successful start executing it
    ...
    def execute_java (self, classname, input):
        tic = time.time()

        args = " ".join(map(str, input))
